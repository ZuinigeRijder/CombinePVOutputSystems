...
GET_SOURCE1_URL = PVOUTPUT_GET_URL + \
    '?key=' + PVOUTPUT_SOURCE1_API_KEY + \
    '&sid=' + PVOUTPUT_SOURCE1_SYSTEM_ID + \
    '&h=1&asc=0&limit=288'
GET_SOURCE2_URL = PVOUTPUT_GET_URL + \
...
def addBatchStatus(dataPointsCount, dataPointsStr) -> str:
    ...
    while True:
        retry = retry + 1
        content = post(PVOUTPUT_ADD_URL, dataPointsStr, TARGET_HEADER)
...
def readInMemory(prefix, content, dateString, targetMinutes, lastWrittenWh):
    ...
    while i >= 0:
        ...
        d, t, whStr, energyEfficiency, w, \
            averagePower, normalisedOutput, energyConsumption, \
            PowerConsumption, temp, v = \
            line.split(',')
    ...
    log(prefix +
        ' readInMemory last values: written time=' + lastWrittenTime +
        ', written wh=' + str(lastWrittenWh) + ', wh=' + str(prevWh) +
        ', line=' + prevLine)
...
target = getStatus('Target  ', GET_TARGET_URL, dateStr, '05:00')
lastTMinutes = computeMinutes(dateStr, target)
source1Content = getStatus('Source 1', GET_SOURCE1_URL, dateStr, '05:00')
lastS1Minutes = computeMinutes(dateStr, source1Content)
source2Content = getStatus('Source 2', GET_SOURCE2_URL, dateStr, '05:00')
lastS2Minutes = computeMinutes(dateStr, source2Content)

source1Dict, lastWrittenSource1Wh = \
    readInMemory('Source 1', source1Content, dateStr, lastTMinutes, 0)
source2Dict, lastWrittenSource2Wh = \
    readInMemory('Source 2', source2Content, dateStr, lastTMinutes, 0)
...
while True:
    ...
    now = datetime.now()
    if now.hour < 5 or now.hour > 22:  # only check between 5 and 23 hours
        log('Outside solar generation hours (5..23)')
        sys.exit('Exiting program to avoid possible memory leaks')

    timeStr = minutesToTimeString(lastTMinutes)
    source1Content = getStatus('Source1', GET_SOURCE1_URL, dateStr, timeStr)
    lastS1Minutes = computeMinutes(dateStr, source1Content)
    source1Dict = {}
    if lastS1Minutes > lastTMinutes:
        source1Dict, nop1Wh = \
            readInMemory(
                'Source 1', source1Content, dateStr,
                lastTMinutes, lastWrittenSource1Wh
            )
    elif now.hour != 22:
    ...
    source2Content = getStatus('Source2', GET_SOURCE2_URL, dateStr, timeStr)
    lastS2Minutes = computeMinutes(dateStr, source2Content)
    source2Dict = {}
    if lastS2Minutes > lastTMinutes:
        source2Dict, nop2Wh = \
            readInMemory(
                'Source 2', source2Content, dateStr,
                lastTMinutes, lastWrittenSource2Wh
            )
    elif now.hour != 22:
        # when after 22:00 if there are results, process them anyway
        # because other inverter might be sleeping
    ...
    if lastS1Minutes <= lastTMinutes and lastS2Minutes <= lastTMinutes:
        continue  # nothing to do
    ...
    while fromMinutes <= toMinutes:
        ...
        if fromMinutes > lastTMinutes:
            # only write when AFTER last target time
            pvoutputString = \
                d + ',' + t + ',' + \
                str(wh) + ',' + str(w) + ',,,,' + str(round(v))
            dataPointsCount += 1
    ...
    lastTMinutes = min(lastS1Minutes, lastS2Minutes)
