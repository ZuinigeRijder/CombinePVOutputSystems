...
GET_SOURCE_1_URL = (
    PVOUTPUT_GET_URL
    + "?key="
    + PVOUTPUT_SOURCE1_API_KEY
    + "&sid="
    + PVOUTPUT_SOURCE1_SYSTEM_ID
    + "&h=1&asc=0&limit=288"
)
GET_SOURCE_2_URL = (
    PVOUTPUT_GET_URL
    + "?key="
    + PVOUTPUT_SOURCE2_API_KEY
    + "&sid="
    + PVOUTPUT_SOURCE2_SYSTEM_ID
    + "&h=1&asc=0&limit=288"
)
GET_TARGET_URL = (
    PVOUTPUT_GET_URL
    + "?key="
    + PVOUTPUT_TARGET_API_KEY
    + "&sid="
...
def add_datapoints(datapoints_count, datapoints_str) -> str:
    ...
    while True:
        retry += 1
        request = Request(PVOUTPUT_ADD_URL, data=post_data, headers=TARGET_HEADERS)
...
def process_one_pvoutput_result(
    ...
    (date_str, time_str, watthour_str, _, watt_str, _, _, _, _, _, volt_str) = (
        line.split(",")
    )
    ...
    if watthour_str == "NaN":
        return 0, 0, "0", ""  # not interested when Watt is not filled
...
def read_entries_in_memory_with_5_minutes_intervals(
    prefix, content, written_target_minutes, written_watthour
) -> tuple[int, dict]:
    ...
    while i >= 0:
        ...
        minutes, prev_wh, prev_volt_str, line = process_one_pvoutput_result(
            line,
            (prev_line, prev_wh, prev_volt_str),
    ...
    log(
        prefix
        + " read_in_memory since "
        + since_time
        + ": written="
        + written_hhmm
        + ", written wh="
        + str(written_watthour)
        + ", wh="
        + str(prev_wh)
        + ", line="
        + prev_line
...
def compute_combined_values(
    source_1_dict, source_2_dict, from_minutes, written_watthour_1, written_watthour_2
    ...
    return (watthour, watt, volt, written_watthour_1, written_watthour_2)


# == compute_from_and_to_minutes =============================================
def compute_from_and_to_minutes(source_1_dict, source_2_dict) -> tuple[int, int]:
...
def process_sources_entries(
    ...
    from_minutes, to_minutes = compute_from_and_to_minutes(source_1_dict, source_2_dict)
...
def main_loop():
    ...
    while True:
        ...
        written_minutes_1, _, source_1_dict = compute_entries_since(
            "Source 1", GET_SOURCE_1_URL, written_target_minutes, written_watthour_1
        )
        ...
        written_minutes_2, _, source_2_dict = compute_entries_since(
            "Source 2", GET_SOURCE_2_URL, written_target_minutes, written_watthour_2
        )
        ...
        if (
            written_minutes_1 <= written_target_minutes
            and written_minutes_2 <= written_target_minutes
        ):
            continue  # nothing to do

        written_watthour_1, written_watthour_2 = process_sources_entries(
            source_1_dict,
            source_2_dict,
            written_target_minutes,
            written_watthour_1,
